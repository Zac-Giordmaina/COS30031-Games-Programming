        ...
        if (validCommand(words[0])) {
            ...
            if (words[0] != "" && words.size() == 1) {
                ...
                for (int i = 0; i < wordsSize; i++) {
                    if (words[i] == "take" || words[i] == "remove" || words[i] == "consume"
                        || words[i] == "put" || words[i] == "open" || words[i] == "destroy"){
                ...
                } else if (words[0] == "consume") {
                    consumeItem(sentence);
                } else if (words[0] == "destroy") {
                    if (commands.at(0)->execute(words[0])) {
                        destroyItem(sentence);
                    } else {
                        cout << "\nYou do not possess such a power" << endl;
                    }
                } else {
...
void CommandManager::consumeItem(string item) {
    Elixir tempElixir = commands.at(1)->executeConsume(item);

    if (tempElixir.getName() != "null") {
        commands.at(1)->executeConsume(tempElixir);
        commands.at(0)->executeConsume(tempElixir);
    } else {
        cout << "\nThis item can not be consumed" << endl;
    }
}

void CommandManager::destroyItem(string item) {
    InventoryItem tempItem = commands.at(1)->execute("take", item);

    if (tempItem.getName() != "fail") {
        commands.at(1)->execute("remove", "", tempItem);
        cout << "\n" + item + " has been destroyed!" << endl;
    } else {
        cout << "\nThis item can not be destroyed" << endl;
    }
}
