...
void Player::execute(string key, string itemKey, InventoryItem item) {
    if (key == "add") {
        if (item.getKey() != "power") {
            ...
            playerMessenger->addToTakeBlackboard();
        }
    } else {
...
void Player::addMessenger(Messaging &messenger) {
    playerMessenger = &messenger;
}

void Player::reducePlayerEnergy(int value){
    setEnergy(value);
}
...
void Player::moveAnnouncement(int value) {
    setEnergy(value);
}
