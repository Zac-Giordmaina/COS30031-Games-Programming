...
void Location::printElixir() {
    if (locationElixir.getName() != "null") {
        cout << locationElixir.getName() + ", (" + locationElixir.getDescription() + ")" << endl;
    }
}
...
void Location::setupLocationEntities(vector<string> entities) {
    ...
    for (int i = 0; i < entities.size(); i = i + 3) {
        if (!hasBag && !hasElixir) {
            ...
            locationElixir = Elixir(entities[i], entities[i + 1], entities[i + 2]);
        }
    }
...
Elixir Location::getElixir() {
    return locationElixir;
}
...
void Location::removeElixir() {
    locationElixir = Elixir();
}
